...
VALUES 
    ...
    JOIN shops
    ON shops.id = cats.shops_id
    WHERE NOT cats.name = 'Zuza' AND NOT cats.name = 'Murzik';
    
    -- 4.Вывести название и цену для всех анализов, которые продавались 5 февраля 2020 и всю следующую неделю.
    -- не понял задание, по этому выполнил как смог :)
    -- var2
    SELECT Analysis.an_price, test1.*
    FROM Analysis
    JOIN
	(SELECT GroupsAn.gr_id, GroupsAn.gr_name, anid.ord_datetime
    FROM GroupsAn
    JOIN 
    (SELECT Orders.ord_datetime, GroupsAn.gr_id 
    FROM Orders 
    JOIN GroupsAn 
    WHERE Orders.ord_datetime > '2020-02-05') as anid
    ON GroupsAn.gr_id = anid.gr_id) AS test1
    ON Analysis.an_id = test1.gr_id;
